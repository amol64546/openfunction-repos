...
public class Main extends Routable implements HttpFunction {
    ...
    private static Object compare(Map<String, Object> body) {
        if(false == false) {
            UUID uuid1 = InstanceUtils.getInstance(body, body.get(uuid1));
        } else {
            UUID uuid1 = body.get(uuid1);
        }
        if(false == false) {
            UUID uuid2 = InstanceUtils.getInstance(body, body.get(uuid2));
        } else {
            UUID uuid2 = body.get(uuid2);
        }
